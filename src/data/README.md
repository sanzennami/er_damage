...
### 「承受伤害增加」怎么建模：先看它放大谁的伤害

游戏里有两类「让敌人承受的伤害增加」，**建模方式不同，不要混用**：

| 类型 | 判据 | 做法 |
| --- | --- | --- |
| **易伤类** | 放大**所有来源**的伤害，队友打上去也享受 | `specialSkillRules.json` 的 `modifiers` 开关，选项里给 `damageBonus` |
| **自增伤类** | 只放大**自己特定几段**的伤害 | 在受影响的技能条目上写 `selfAmp`，页面在总计下面多一行 |

易伤类的例子是秀雅 Q 冲撞点（站在上面的敌人承受伤害 +30%）——它是全局乘区，
做成开关，开了之后这名实验体的所有段都跟着涨。

自增伤类的例子是彰一 E 协商（被标识的敌人**因投掷的短剑**承受伤害 +25%）——
只影响他自己的 R 短剑和 P 短剑两段，做成开关既容易忘了关、也没法只作用于部分段，
所以直接写进那两段：

```jsonc
"selfAmp": {
  "source": "协商",              // 增伤来自哪个技能，用来拼行标题
  "multiplier": 1.25,           // 加成后的倍率，不是增量
  "note": "被 E 协商标识的敌人（5 秒）……"   // 可选，显示在这一段下方
}
```

页面效果是在「单目标 / N 目标总计」那行下面多一行 **「受协商增伤时伤害」**，
用冷色边框和总计区分开，免得被当成这一段本身又打了一次。原始值和最终值都乘上倍率。
