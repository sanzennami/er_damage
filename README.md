...
| `targetHp` | 目标体力上限 |
| `targetCurrentHp` / `targetLostHp` | 目标当前 / 已失体力（界面滑条推出） |
| `maxHp` | **自身**体力上限；官方写「体力上限」不带「目标」时用这个 |
| `selfCurrentHp` / `selfLostHp` | 自身当前 / 已失体力（界面滑条推出） |
| `extraHp` | 自身额外体力（装备 + 潜能那部分） |
| `defense` / `shield` | 自身防御力 / 护盾量 |
| `critChance` | 暴击率（0~1） |
| `basicAttackAmp` | 普攻增幅（0~1），只作用于普攻那条乘区 |
| `accumulatedDamage` | 累计伤害，界面手填（丹尼尔 W 用） |
| `stacks` | 界面叠层输入 |
| `level` | 当前技能等级，从 1 开始 |
...
普攻那条是另一套乘区（官方 Wiki 口径）：
普攻增幅   baAmp     = 装备普攻增幅 + 熟练度普攻增幅 + 特效普攻增幅
普攻伤害             = floor((攻击力 * defenseMod * 暴击修正 + 普攻额外伤害) * (1 + 自身增伤 + baAmp - 目标普攻减伤))
                       暴击倍率 = 1.75 + 暴击伤害；「普攻额外伤害」是防御与暴击之后才加的固定值
```

**技能增幅和普攻增幅是互不相干的两个桶。** 被官方判定为普攻伤害的技能段
（条目上 `damageType: "basicAttack"`，如李黛琳醉仙2段、莉央替弓、艾登 Q）走上面那条普攻线，
吃普攻增幅和普攻减伤，而不是技能减伤。

目标熟练度带来的减伤：技能 `等级 * 0.8%`（2 级起算）、普攻 `等级 * 1%`（1 级 1%，20 级 20%）。
